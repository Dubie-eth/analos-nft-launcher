-- Supabase Security Fixes with CASCADE for Dependencies
-- This script handles function dependencies properly
...
-- 6. Drop dependent triggers first, then functions with CASCADE
-- Drop triggers that depend on functions
DROP TRIGGER IF EXISTS update_leaderboard_trigger ON public.user_profiles;
DROP TRIGGER IF EXISTS trigger_update_leaderboard ON public.user_profiles;
...
DROP FUNCTION IF EXISTS public.calculate_verification_score(text) CASCADE;
DROP FUNCTION IF EXISTS public.check_verification_eligibility(text) CASCADE;
DROP FUNCTION IF EXISTS public.get_encryption_key() CASCADE;
DROP FUNCTION IF EXISTS public.get_user_rank(text) CASCADE;
DROP FUNCTION IF EXISTS public.increment_activity_points(text, integer) CASCADE;
DROP FUNCTION IF EXISTS public.increment_referral_points(text, integer) CASCADE;
...
DROP FUNCTION IF EXISTS public.get_referral_history(text) CASCADE;
DROP FUNCTION IF EXISTS public.get_campaign_stats() CASCADE;
DROP FUNCTION IF EXISTS public.check_user_eligibility(text) CASCADE;
DROP FUNCTION IF EXISTS public.get_activity_history(text) CASCADE;
DROP FUNCTION IF EXISTS public.cleanup_old_security_logs() CASCADE;
DROP FUNCTION IF EXISTS public.get_top_referrers(integer) CASCADE;
DROP FUNCTION IF EXISTS public.get_security_statistics() CASCADE;
DROP FUNCTION IF EXISTS public.calculate_creator_rewards(text) CASCADE;
DROP FUNCTION IF EXISTS public.update_collection_sales(text, numeric) CASCADE;
DROP FUNCTION IF EXISTS public.get_user_rewards_summary(text) CASCADE;
DROP FUNCTION IF EXISTS public.check_wallet_eligibility(text) CASCADE;
DROP FUNCTION IF EXISTS public.add_to_whitelist(text, text) CASCADE;
DROP FUNCTION IF EXISTS public.handle_new_user() CASCADE;
DROP FUNCTION IF EXISTS public.cleanup_old_data() CASCADE;

-- 7. Recreate functions with proper security settings
...
-- 8. Create indexes to improve performance
...
-- 10. Set up triggers for updated_at columns (if not already exists)
DO $$
BEGIN
