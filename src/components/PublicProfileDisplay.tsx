    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        if (profileResponse.ok) {
          ...
          const nftValue = enhancedProfile.nfts?.reduce((sum, nft) => sum + (nft.price || 0), 0) || 0;
          const tokenValue = enhancedProfile.tokens?.reduce((sum, token) => sum + (token.balance * token.price), 0) || 0;
